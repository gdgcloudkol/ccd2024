...
import { ATTENDEES_DJANGO_URL, EVENT_ATTENDEE_UPDATE_URL_SUFFIX } from '@/lib/constants/be';
...
export async function POST(req: NextRequest, { params }: { params: { attendeeId: number } }) {
    ...
    const response = await bkFetch(`${ATTENDEES_DJANGO_URL}${params.attendeeId}${EVENT_ATTENDEE_UPDATE_URL_SUFFIX}`, {
        method: "POST",
        cache: 'no-store',
