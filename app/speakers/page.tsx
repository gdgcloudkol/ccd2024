...
export default function Speakers() {
  const peopleList: GridData[] = speakerList.allPeople as any;
  const extendedEventPeopleList: GridData[] =
    speakerList.extendedEventSpeakers as any;
  ...
  const getNormalSpeaker = async () => {
    ...
    try {
      ...
      const filteredData = data.filter(
        (speaker: any) => !filterIds.includes(speaker.id)
      );
      setSpeakers(() =>
        filteredData.map((speaker: any) => {
          let linkedin = speaker.links.find(
            (link: any) => link.title === "LinkedIn"
          )?.url;
          let twitter = speaker.links.find(
            (link: any) => link.title === "Twitter"
          )?.url;

          if (!linkedin) linkedin = speaker?.questionAnswers?.[2]?.answer;
          if (!twitter) twitter = speaker?.questionAnswers?.[1]?.answer;
      ...
      <section className='flex flex-col local-container gap-6 p-4 space-y-4'>
        {!disabledSpeakersContent.speakers ? (
          ...
          peopleList.map((people: GridData, index: number) => {
            return (
              <CardGrid key={people.title} gridData={people} type='People' />
            );
          })
        )}
        {speakers.length > 0 && (
          <CardGrid
            gridData={{
              ...
              description:
                "Discover our amazing speakers for the final day of GCCD Kolkata 2024",
              people: speakers,
            }}
            type='People'
          />
        )}
        {extendedEventPeopleList.map((people: GridData, index: number) => {
          return (
            <CardGrid key={people.title} gridData={people} type='People' />
          );
        })}
