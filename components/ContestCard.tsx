...
function ContestCard({ cards }: { cards: ContestCards[] }) {
  return cards.map((card: ContestCards) => (
    <div
      key={card.id}
    ...
    >
      <a href={card.link}>
        ...
        <div className='p-4 flex flex-col items-center pb-10'>
          ...
          <p className='text-gray-700 text-sm mb-2 min-h-14'>{card.content}</p>
          <div className='flex items-center justify-center'>
            ...
            <button className='bg-google-blue p-2 rounded-lg mt-5'>
              Start Contest
            </button>
          </div>
        </div>
